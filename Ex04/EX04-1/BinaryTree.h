...
//  二叉树方法声明及重载编译系统默认方法声明:
	...
	void Revolute(BinTreeNode<ElemType> *p);//左右子树交换
...
template <class ElemType>
void Leaf(BinTreeNode<ElemType> *p, int& leafcount){
	...
	Leaf(p->leftChild,leafcount);
	Leaf(p->rightChild,leafcount);
}
...
void Revolute(BinTreeNode<ElemType>*p){
	if(p!=NULL){
		...
		ElemType temp;
			temp=p->leftChild->data;
			p->leftChild->data=p->rightChild->data;
			p->rightChild->data=temp;
		}
